...
eMax = input("Inserire energia massima:")
print "Attendere: "
a = np.array([])
c = 0
for i in range(0, int(eMax * 100), 1):
    j = (i / 100.0) + 1 / 1000.0;
    ...
    if i % 100 == 1:
        print int(eMax) - c + 1
print "Gli stati fisici della funzione d'onda sono: ", a
...
err = 0
while err == 0:
    en = input("Indicare la posizione nell'array dell'energia per la quale si vuole calcolare il numero di zersi della funzione d'onda:")
    if en-int(en) != 0 or en < 0 or en > len(a):
        print "Non valido"
    else:
        err = 1
b = 0
for i in range(0, 14000, 1):
    if psi[i] * psi[i - 1] < 0:
        b = b + 1
print "La funzione d'onda ha ", b, " zeri"
